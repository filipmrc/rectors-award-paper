...
ROS paket \texttt{ros\_control}ostvaruje generičke PID regulatore unutar ROS arhitekture. 
...
Gazebo simulaciju moguće je potpuno upravljati kroz ROS koristeći \texttt{ros\_control}sučelje, na isti način kao što upravljamo stvarnim sustavom.
...
Radi potrebe za općim rješenjem koristimo Moore-Penroseov pseudoinverz, koji je svojevrsno poopćenje inverza za sve matrice.
...
Ovom modifikacijom poopćavamo ideju izraza \ref{jac_approx2} na sve manipualtore.
Programskom izvedbom pomoću KDL biblioteka na izraz \ref{jac_approx2} dodaju se članovi kojima osiguravamo numeričku stabilizaciju prema metodi opisanoj u (), no za dokaz koncepta u Matlabu koristimo jednostavniju izvedbu.
...
U slučaju dostupnosti sklopovskog sučelja koje dopušta direktno slanje naredbi kartezijske brzine, algoritam se znatno pojednostavljuje jer ne zahtjeva kinematičke kalkulacije.
...
Vektor razlike konfiguracija sklairamo za neki iznos $\alpha$, ali daljnju regulaciju ostavljamo upravljačkom algoritmu samog manipulatora, što znači da \texttt{ros\_control}regulator ovoj varijanti koristimo bez povratne veze.
...
\subsubsection{Sučelje brzine zglobova}
...
Nakon inicijalizacije samog manipulatora, sučelje registrira pojedinačne zglobove i "spaja" se na naredbe pisanja i čitanja stanja zglobova.
...
Važno je spomenuti da svaki regulator ima odgovarajuću podvrstu \verb|hardware_interface| klase i neće se spajati na neodgovarajuća sučelja.
...
Kako bi rješili problem kinematike opisan u poglavlju \ref{direkt i inverz}, u klasu \texttt{KDL::Chain} učitavamo opis kinematičkog lanca iz \texttt{.urdf} datoteke.
Iz učitanog se lanca pomoću dostupnih KDL metoda dostupnih klasi \texttt{Kinematics} generira matrica direktne kinematike i Jakobijan sustava. 
Nakon toga i funkcija koja obavlja množenje pseudoinverza jakobijana traženom promjenom vektora konfiguracija način sličan \ref{jac_approx2}, pri tome dodajući članove koji osiguravaju numeričku stabilnost.
...
Dobiveni vektor razlike konfiguracije se pomoću funkcije \texttt{KDL::ChainIkSolverVel\_wdls::CartToJnt} pretvara u vektor pogreške pozicije zglobova.
