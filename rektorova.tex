...
Zahvaljujemo se prof. dr. sc Ivanu Petrovići i prof. dr. sc. Zoranu Kalafatiću na ukazanom povjerenju.
...
\chapter{Rezultati}
Ukupnu izvedbu upravljačko sučelja temeljito je testirano u nekoliko faza. 
Prvo je bilo potrebno uvjeriti se da se upravljačka petlja ponaša na odgovarajući način pri praćenju putanja zadanih RGB-Dubinskom kamerom.
Ovo provjeravamo simuliranjem cijelokupnog sustava unutar Gazebo okruženja, što možemo bez dodatnih modifikacija zahvaljujući \texttt{ros\_control} paketu.
Uvjerivši se u pouzdanost sustava, prelazimo na testiranje na pravoj Jaco robotskoj ruci.
Tesiranje na stvarnoj ruci prvo provodimo praćenjem fiksnih periodičnih trajektorija, a zatim prelazimo na putanje zadane RGB-Dubinskom kamerom.
Konačno, ispitujemo korisnost sustava u usporedbi sa konvencionalnim načinom upravljanja robotskog rukom, tako da korisnicima bez prijašnjeg iskustva zadajemo zadatak koji moraju obaviti sa obje metode.

\section{Praćenje putanje u simulaciji}
Unatoč tome što smo u poglavlju \ref{Upravljački algoritam} dokazali kinematičku stabilnost izvođenja putanje, provjeravamo robusnost sustava pri kretanju ruke korisnika.
...
Tokom testiranja primjećeno je da se najbolji rezulati dobivaju skaliranjem ove vrijdnosti na valjanje alata u rasponu od $[-45^{\circ} , 45^{\circ}]$.
\begin{figure}[H]
\centering
...
Ovaj problem nestaje pri višim brzinama karakterističnim za kretanje ljudske ruke, stoga ovaj rezultat uzimamo kao izrazito pozitivan.
...
Pri tome je bitno napomenuti kako je radni prostor robotske ruke u oba eksperimenta ograničen po z osi, kako ne bi došlo do udaranju u podlogu pri ispitivanju.
...
Na izraženim krivuljama vidljivo je kako u pri ovakvom kretanju putanja samomg manipulatora zadržava oblik putanje korisnikove ruke.
...
\chapter{Zaključak}
Sustav zamišljen kao intuitivniju alternativu konvencionalnim načinima upravljanja ostvarili smo u potpunosti koristeći metode neovisne o konkretnoj sklopovskoj implementaciji. 
Smatramo da smo ovakvim pristupom prikazali moguće rješenje problema ovakvog tipa upravljačkog sučelja u općem slučaju.

Problem kinematike detaljno smo obradili kako bi prikazali prepreke na koje se nailazi pri poopćenju.
Inverznu kinematiku rješili smo iterativno na način primjenjiv za velik broj manipulatora, pritom uzimajući u obzir numeričke karakteristike rješenja te ograničenja matematičkog modela.
Matematički model zatim smo uspješno testirali i pri tome objasnili potrebu za primjenom numerički robusnijeg algoritma, koji smo potom i ostvarili.

Detekciju pozicije i orijentacije ljudskog dlana također smo ostvarili na dva načina.
Prvi je način komputacijski vrlo učinkovit no ostavlja prostora za razvoj radi nemogućnosti pronalaženja orijentacije ruke korisnika.
...
Konačno, ova dva algoritma ujedinili smo u upravljačku strukturu više razine baziranu na ROS arhitekturi.
Prednost naše arhitekture leži u univerzalnosti sučelja te vrlo lakoj prenamjeni.
Cjelokupnu upravljačku arhitekturu ispitujemo u Gazebo programskom sučelju i zaključujemo kako su rezultati zadovoljavajući za primjenu.
