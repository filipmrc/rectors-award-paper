...
Za složenije manipulatore ovaj problem je netrivijalan te rješenje nije uvijek moguće pronaći.
...
Analitička rješenja problema ne postoje za većinu konfiguracija sa 6 stupnjeva slobode \cite{grochow2004style}, a klasični pristupi aproksimiranju rješenja sustava nelinearnih jednadžbi \cite{broyden1965class}\cite{bi2009three} zahtjevaju složen račun.
...
U prvom odlomku opisati ćemopreskočiti pojedinosti D-H postupka i izvod matrica homogene transformacije.
...
Pri tome ćemo radi sažetosti izlaganja  koje koristimo u računalnom algoritmu.
...
\begin{equation}
\Delta \bm{\theta} = \mathbf{J}^T\left(\mathbf{J}^T \mathbf{J} + \lambda^2\mathbf{I}\right)^{-1} \label{dlq2}
\end{equation}
Pokazalo se da je ovakva izvedba superiorna prema ostalim metodama, što pokazujemo u odlomku \ref{provjera}.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.49]{matlab_pravocrtno1}
\caption{Kretanje matematičkog modela Jaco robotske ruke prema zadanoj točki. Orijentacija alata je konstantna.} \label{P1}
\end{figure}
...
%ubaci vremena
Kretanje koje uključuje rotacije pokazalo se nešto većim izazovom za predloženu metodu.
\begin{figure}[h!]
\centering
...
Pronalazak svih singularnih konfiguracija zahtjeva pronalaženje svih zakreta zglobova koji zadovoljavaju identitet \ref{singularity}:
\begin{equation}
\det \mathbf{J} = \mathbf{0} \label{singularity}
\end{equation}
