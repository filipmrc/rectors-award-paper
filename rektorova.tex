...
Pri primanju poruke tipa \verb|ime_poruke| sa detekcijskog servera, funkcija \verb|sendCartesianCommand| unutar glavnog čvora obrađuje podatke. 
Vektor razlike konfiguracija sklairamo za neki iznos $\alpha$, ali daljnju regulaciju ostavljamo upravljačkom algoritmu samog manipulatora, što znači da \verb|ros_control| regulator ovoj varijanti koristimo bez povratne veze.
...
Samo sučelje jest programska rutina koji ostvarujemo tako da API funkcije manipulatora "umotavamo" u \verb|hardware_interface| klasu koja je dio \verb|ros_control| paketa.
...
Ovo spajanje vrši se tako da se spomenuti API naredbe umataju u funkcije čiji su pokazivači dostupni  odgovarajućim \verb|ros_control| regulatorima.
...
Sljedeći korak je inicijalizacija sučelja, pokreće se regulator tipa \verb|velocity_controllers/JointGroupVelocityController|. 
Regulator svoje parametre učitava iz odgovarajuće konfiguracijske (.yaml) datoteke.
...
Pokretanjem glavnog čvora dovršavamo inicijalizaciju sustava.
Dok \verb|ros_control| regulator osigurava stabilnu brzinu, glavni čvor čini nadređenu regulacijsku petlju koja osigurava odgovarajuću referentnu veličinu.


Kako bismo ostvarili upravljanje samo preko sučelja brzine zglobova, morali smo rješiti problem kinematike iz poglavlja \ref{direkt i inverz}.
Uz ovu vrstu sučelja odabranu pri pokretanju, glavni čvor nakon sinteze vektora razlike konfiguracija.
