...
\begin{itemize}
  ...
  Nakon toga se rezultat pretvara u potrban zakret (ili brzinu zakreta) zglobova manipulatora, nakon čega se šalje u API sučelje.
...
Također, mrežna arhitektura-ROS omogućuje pokretanje komponenti pojedinačno na različitim fizičkim računalima, što se pokazalo korisnim.
...
Pomoću ove biblioteke moguće je programski rješavati direktnu i inverznu kinematiku te dinamiku manipulatora sa do 7 stupnjeva slobode.
...
U idealnom slučaju, rješavanje problema kinematike manipulatora proizvoljnog broja osi zahtjeva sintezu matrica homogene transformacije između koordinatnih sustava baze i alata.
...
Rješavanje problema direktne kinematike u općem je slučaju jednostavno: D-H postupkom postavljamo koordinatne sustave vezane uz zglobove ,a potom određivanjem matrica homogenih transformacija dobivamo jednoznačno rješenje.
Problem inverzne kinematike znatno je složeniji jer zahtjeva rješavanje sustava nelinearnih jednadžbi s više nepoznanica, što je vrlo često netrivijalno.
...
Pronalaženje analitičkih rješenja inverzne kinematike pomoću računalnog algoritma vrlo je složen zadatak, pogotovo uzevši u obzir činjenicu da svako rješenje nije nužno fizički moguće.
...
Detektor je implementiran u obliku Python skripte koja poziva naredbe iz OpenCV biblioteke te \textit{freenect} biblioteku za dohvat dubinske slike.\\\\
Razvijeni detektor sastoji se od više koraka te je sposoban detektirati položaj ruke, orijentaciju dlana te otvorenost šake.\\\\
...
\end{equation}
\section{Izdvajanje dlana od okoline}
Prvi korak detektora je izdvojiti dlan na slici. Kao i kod metode Qiana i suradnika \cite{qian2014realtime}, ali i mnogih drugih, i u ovom radu se pretpostavlja kako se ruka nalazi na najmanjoj dubini (niti jedan predmet u vidnom polju kamere nije bliži od ruke).\\\\

Za razliku od spomenute metode, u ostvarenoj metodi nije potrebno koristiti pomagala kao što je nereflektirajuća crna narukvica na zapešću. Izolacija dlana od okoline vrši se jednostavnim dubinskim filterom na empirijski određenoj dubini od $130mm$ u odnosu na globalni minimum dubine slike, što je ujedno i globalni minimum dijela slike na kojem se nalazi ruka. Iako najveći raspon prosječne ruke premašuje $110mm$, to ne predstavlja prevelik problem.\\\\
...
Pri tome također valja imati na umu da na primjer gradijent u običnoj slici ima značenje gradijenta boje, dok se u dubinskoj slici odnosi na gradijent dubine.\\\\
Neki od ključnih postupaka u detekciji izvedeni su pomoću ove biblioteke:
\begin{enumerate}[label=$\bullet$]
	\item \textbf{Segmentacija slike na osnovu praga} je postupak prilikom kojeg se s obzirom na željenu vrijednost praga može konstruirati binarna slika (maska) gdje jedna vrijednost pokriva sve elemente čija je vrijednost ispod praga, a druga vrijednost sve elemente iznad praga. Na slici \ref{fig:image_segmentation} (b) vidimo binarnu sliku dobivenu segmentacijom po dubini slike (a) pri čemu je prag određen kao $130mm$ u dubinu od minimalne vrijednosti dubine na slici.
