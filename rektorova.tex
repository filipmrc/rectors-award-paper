...
Kako bi primijenio postojeći sustav na drugog robota (uz dostupnost istog seta naredbi koje regulator koristi), korisnik mora samo uklopiti odgovarajuće API funkcije unutar generičkih.
...
\section{TensorFlow}
\textit{TensorFlow} je biblioteka otvorenog koda namijenjena brzom izračunu operacija nad podacima u obliku višedimenzionalnih tenzora. Temelji se na grafovima toka podataka gdje svaki čvor (engl. \textit{node}) u usmjerenom grafu je jedna operacija nad jednim ili više ulaznih tenzora te računa izlaz kao jedan ili više izlaznih tenzora. Većina operacija ima zna izračunati i parcijalnu derivaciju svakog izlaza po svakom od ulaza koje se koriste prilikom propagacije greške tokom učenja. 
...
Blok $\mathbf{J^{-1}}$ u ovom slučaju podrazumijeva bilo koju od metoda \ref{jac_approx2}, \ref{jacob3} ili \ref{dlq2} kojima povezujemo promjene zglobova sa promjenama vektora konfiguracije.
\begin{figure}[!h]
\centering
...
Vektor funkcija $\mathbf{f}$ u ovom slučaju označava preračunavanje kutova očitanih sa manipulatora ruke $\bm{\theta_r}$ u kuteve koji odgovaraju postavljenom matematičkom modelu $\bm{\theta}$. 
...
Sa slike \ref{rot1m} vidi se kako pri isključivo rotacijskom kretanju korištenjem izraza \ref{jac_approx2} ne može se dobiti zadovoljavajuće rješenje.
...
Kako bi riješili problem kinematike opisan u poglavlju \ref{direkt i inverz}, pomoću metoda dostupnih u KDL bibliotekama učitavamo opis kinematičkog lanca iz \texttt{.urdf} datoteke.
